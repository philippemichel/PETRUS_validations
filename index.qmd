...
vvn <- var.test(zz$value~zz$name)
vvn <- beaup(vvn$p.value, affp = 1)

vvg <- var.test(zg$value~zg$name)
...
Pour la mesure du nerf  optique la technique
`à l'interface vitrée` a tendance est être plus petite que la mesure `habituelle`. Pas de différence pour la mesure de la gaine.Mais la précision est la même, il n'y a pas de différence dans les variances.
